...
fn main() {
    ...
    let mut inputfile = File::open("inputs/section_1/bad_upwind/exec_bad_upwind.yml")
        .unwrap_or_else(|err| {
    ...
    let mut outputfile =
        File::create(format!("{}/exec_bad_upwind.dat", dir_str)).unwrap_or_else(|err| {
            eprintln!("Problem creating output files: {}", err);
            process::exit(1);
        });

    // run
    bad_upwind::run(&input_params, DiffMethod::Forward, &mut outputfile).unwrap_or_else(|err| {
